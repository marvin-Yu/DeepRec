...
// form; e.g. see DfsNloVisitor.

#ifndef NLO_INTERFACE_NLO_INSTRUCTION_H_
#define NLO_INTERFACE_NLO_INSTRUCTION_H_
...
class NloInstruction {
 public:
  ...
  static std::unique_ptr<NloInstruction> CreateParameter(int64_t parameter_number,
                                                         const NloShape& shape,
                                                         const string& name);

  // Creates a literal constant instruction.
  static std::unique_ptr<NloInstruction> CreateConstant(
      std::unique_ptr<NloLiteral> literal);

  // Creates a get tuple element instruction.
  static std::unique_ptr<NloInstruction> CreateGetTupleElement(
      const NloShape& shape, NloInstruction* operand, int64_t index);

  // Creates a trace instruction that logs the input operand in the computation.
  static std::unique_ptr<NloInstruction> CreateTrace(const string& tag,
                                                     NloInstruction* operand);

  // Creates a unary instruction (one operand).
  // Precondition: opcode must be a legitimate unary operation.
  static std::unique_ptr<NloInstruction> CreateUnary(const NloShape& shape,
                                                     NloOpcode opcode,
                                                     NloInstruction* operand);

  // Creates a binary instruction (two operands).
  // Precondition: opcode must be a legitimate binary operation.
  static std::unique_ptr<NloInstruction> CreateBinary(const NloShape& shape,
                                                      NloOpcode opcode,
                                                      NloInstruction* lhs,
                                                      NloInstruction* rhs);
  ...
  static std::unique_ptr<NloInstruction> CreateTernary(const NloShape& shape,
                                                       NloOpcode opcode,
                                                       NloInstruction* lhs,
                                                       NloInstruction* rhs,
                                                       NloInstruction* ehs);
  ...
  static std::unique_ptr<NloInstruction> CreateVariadic(
  ...
  static std::unique_ptr<NloInstruction> CreateMap(
      const NloShape& shape, std::vector<NloInstruction*> operands,
      NloComputation* map_computation,
  ...
  // reduce it to.
  static std::unique_ptr<NloInstruction> CreateReducePrecision(
      const NloShape& shape, NloInstruction* operand, const int exponent_bits,
      const int mantissa_bits);

  // Creates a cross replica sum op.
  static std::unique_ptr<NloInstruction> CreateCrossReplicaSum(
      const NloShape& shape, NloInstruction* operand);

  // Creates a conversion instruction, where operand is the data to convert and
  // shape is the target shape for the conversion.
  static std::unique_ptr<NloInstruction> CreateConvert(const NloShape& shape,
                                                       NloInstruction* operand);

  // Creates an infeed instruction, which reads data of the given shape from the
  // Infeed interface of the device.
  static std::unique_ptr<NloInstruction> CreateInfeed(const NloShape& shape,
                                                      const string& config);

  // Creates an outfeed instruction, which outputs data.
  static std::unique_ptr<NloInstruction> CreateOutfeed(
      const NloShape& shape, NloInstruction* operand,
      string outfeed_config);
  ...
  static std::unique_ptr<NloInstruction> CreateSend(NloInstruction* operand,
                                                    int64_t channel_id);
  ...
  static std::unique_ptr<NloInstruction> CreateRecv(const NloShape& shape,
                                                    int64_t channel_id);

  // Creates a slice instruction, where the operand is sliced by the given
  // start/limit indices.
  static std::unique_ptr<NloInstruction> CreateSlice(
      const NloShape& shape, NloInstruction* operand,
  ...
  // 'slice_sizes'.
  static std::unique_ptr<NloInstruction> CreateDynamicSlice(
      const NloShape& shape, NloInstruction* operand,
      NloInstruction* start_indices,
      std::vector<int64_t> slice_sizes);

  // Creates a dynamic update slice instruction, which updates a slice
  // of 'operand' with 'update' and 'start_indices'.
  static std::unique_ptr<NloInstruction> CreateDynamicUpdateSlice(
      const NloShape& shape, NloInstruction* operand, NloInstruction* update,
      NloInstruction* start_indices);

  // Creates a concatenate instruction, where the operands are concatenated on
  // the provided dimension.
  static std::unique_ptr<NloInstruction> CreateConcatenate(
      const NloShape& shape, std::vector<NloInstruction*> operands,
      int64_t dimension);
  ...
  static std::unique_ptr<NloInstruction> CreateReduce(
      const NloShape& shape, NloInstruction* operand, NloInstruction* init_value,
      std::vector<int64_t> dimensions_to_reduce,
      NloComputation* reduce_computation);

  // Creates a batch-norm-training instruction.
  static std::unique_ptr<NloInstruction> CreateBatchNormTraining(
      const NloShape& shape, NloInstruction* operand, NloInstruction* scale,
      NloInstruction* offset, float epsilon, int64_t feature_index);

  // Creates a batch-norm-inference instruction.
  static std::unique_ptr<NloInstruction> CreateBatchNormInference(
      const NloShape& shape, NloInstruction* operand, NloInstruction* scale,
  ...
  static std::unique_ptr<NloInstruction> CreateBatchNormGrad(
      const NloShape& shape, NloInstruction* operand, NloInstruction* scale,
      NloInstruction* mean, NloInstruction* variance,
      NloInstruction* grad_output, float epsilon, int64_t feature_index);

  // Creates a broadcast instruction.
  static std::unique_ptr<NloInstruction> CreateBroadcast(
      const NloShape& shape, NloInstruction* operand,
  ...
  static std::unique_ptr<NloInstruction> CreateReshape(const NloShape& shape,
                                                       NloInstruction* operand);

  // Creates a transpose instruction.
  static std::unique_ptr<NloInstruction> CreateTranspose(const NloShape& shape,
                                                       NloInstruction* operand);
  ...
  static std::unique_ptr<NloInstruction> CreateWhile(const NloShape& shape,
                                                     NloComputation* condition,
                                                     NloComputation* body,
                                                     NloInstruction* init);
  ...
  static std::unique_ptr<NloInstruction> CreateFusion(
      const NloShape& shape, FusionKind fusion_kind, NloInstruction* fused_root);

  // Creates a call instruction that applies the given computation on the given
  // operands. "shape" is the resultant shape.
  static std::unique_ptr<NloInstruction> CreateCall(
      const NloShape& shape, std::vector<NloInstruction*> operands,
      NloComputation* computation);
  ...
  static std::unique_ptr<NloInstruction> CreateCustomCall(
      const NloShape& shape, std::vector<NloInstruction*> operands,
      std::string custom_call_target);

  static std::unique_ptr<NloInstruction> CreateCustomCall(
      const NloShape& shape, std::vector<NloInstruction*> operands,
      std::string custom_call_target, float epsilon);

  // Creates a tuple instruction with the given elements. This is a convenience
  // wrapper around CreateVariadic.
  static std::unique_ptr<NloInstruction> CreateTuple(
  ...
  static std::unique_ptr<NloInstruction> CreateReverse(
      const NloShape& shape, NloInstruction* operand,
      std::vector<int64_t> dimensions);

  // Returns the opcode for this instruction.
  NloOpcode opcode() const { return opcode_; }
  ...
  const NloInstruction* operand(int64_t i) const;

  // Returns the ith operand to this instruction.
  NloInstruction* mutable_operand(int64_t i);
  ...
  const std::vector<NloInstruction*>& operands() const { return operands_; }

  // Returns the index of 'target' in the operands sequence.
  // Precondition: target must be an operand (or a fatal error will occur).
  int64_t operand_index(const NloInstruction* target) const;
  ...
  const std::vector<NloInstruction*>& users() const { return users_; }

  // Returns true if this instruction is a user of 'instruction'.
  bool IsUserOf(const NloInstruction* instruction) const {
    return NloMapContainsKey(instruction->user_set_, this);
  }
  ...
  NloStatus AddControlDependencyTo(NloInstruction* instruction);

  // Removes a previously added control dependency from this instruction to
  // 'instruction'.
  NloStatus RemoveControlDependencyTo(NloInstruction* instruction);
  ...
  const std::vector<NloInstruction*>& control_predecessors() const {
    return control_predecessors_;
  }
  const std::vector<NloInstruction*>& control_successors() const {
  ...
  bool Identical(
      const NloInstruction& other,
      std::function<bool(const NloInstruction*, const NloInstruction*)>
          eq_operands = std::equal_to<const NloInstruction*>(),
      std::function<bool(const NloComputation*, const NloComputation*)>
          eq_computations = std::equal_to<const NloComputation*>()) const {
    ...
    auto c1 = operands();
    auto c2 = other.operands();
    auto p = std::move(eq_operands);
    if (!((c1.size() == c2.size()) &&
  ...
  // be replaced.
  NloStatus ReplaceUseWith(NloInstruction* user, NloInstruction* new_producer);

  // Replaces the specified operand with new_operand.
  NloStatus ReplaceOperandWith(int64_t operand_no, NloInstruction* new_operand);
  ...
  NloStatus ReplaceAllUsesWith(NloInstruction* new_producer);
  ...
  // call_finish_visit is true, then DfsNloVisitor::FinishVisit is called when
  ...
  NloStatus Accept(DfsNloVisitor* visitor, bool call_finish_visit = true,
  ...
  using CompareFunction =
      std::function<bool(const NloInstruction*, const NloInstruction*)>;
  NloStatus AcceptWithOperandOrder(DfsNloVisitor* visitor,
  ...
  NloStatus Accept(const NloFunctionVisitor::VisitorFunction& visitor_func);
  ...
  int64_t parameter_number() const {
    assert(NloOpcode::kParameter == opcode_);
  ...
  void set_parameter_name(const std::string& str) {
    assert(NloOpcode::kParameter == opcode_);
    parameter_name_ = str;
  }
  ...
  int64_t dimensions(int64_t index) const;

  // Accessor for the dimension in which a concatenate HLO should occur.
  // Precondition: opcode() == NloOpcode::kConcatenate
  int64_t concatenate_dimension() const;

  // Returns the tuple index associated with this instruction.
  //
  // Precondition: opcode() == NloOpcode::kGetTupleElement
  int64_t tuple_index() const;

  // Gets/sets the to_apply NloComputation for Call, Map, Reduce, etc.
  // The setter should only be called by NloModule or NloComputation methods.
  //
  // Precondition: The instruction has a valid to_apply_ field.
  NloComputation* to_apply() const;
  void set_to_apply(NloComputation* to_apply);

  // Returns the custom_call_target for CustomCall.
  // Precondition: opcode() == NloOpcode::kCustomCall
  const string& custom_call_target() const;

  // Returns the config for the Outfeed instruction.
  // Precondition: opcode() == NloOpcode::kOutfeed
  const string& outfeed_config() const;

  // Returns the shape for the Outfeed instruction.
  // Precondition: opcode() == NloOpcode::kOutfeed
  const NloShape& outfeed_shape() const;

  // Gets/sets the while_condition or while_body NloComputation for While. The
  // setters should only be called by NloModule or NloComputation methods.
  //
  // Precondition: The instruction is a While instruction.
  NloComputation* while_condition() const;
  NloComputation* while_body() const;
  void set_while_condition(NloComputation* while_condition);
  void set_while_body(NloComputation* while_body);

  // Gets/sets the select or scatter NloComputation for SelectAndScatter. The
  // setters should only be called by NloModule or NloComputation methods.
  //
  // Precondition: opcode() == NloOpcode::kSelectAndScatter.
  NloComputation* select() const;
  NloComputation* scatter() const;
  void set_select(NloComputation* select);
  void set_scatter(NloComputation* scatter);
  ...
  //
  // Postcondition: retval == nullptr || retval->opcode() == NloOpcode::kTrace
  NloInstruction* tracing() const;
  void set_tracing(NloInstruction* trace_instruction);
  ...
  // channel.
  //
  // Precondition: opcode() == NloOpcode::kSend or NloOpcode::kRecv
  ...
  //
  // Precondition: opcode() == NloOpcode::kBatchNormTraining
  ...
  //
  // Precondition: opcode() == NloOpcode::kTrace
  ...
  //
  // Precondition: opcode() == NloOpcode::kFusion
  NloComputation* fused_instructions_computation() const;
  ...
  //
  // Precondition: opcode() == NloOpcode::kFusion
  NloInstruction* fused_expression_root() const;

  // Returns the list of fused instructions inside this fusion instruction.  The
  // returned type is a range of NloInstruction*s.
  //
  // Precondition: opcode() == NloOpcode::kFusion
  const nlo_iterator_range<NloUnwrappingIterator<
      std::list<std::unique_ptr<NloInstruction>>::const_iterator>>
  fused_instructions() const;

  const nlo_iterator_range<
  ...
  //
  // Precondition: opcode() == NloOpcode::kFusion
  NloInstruction* fused_parameter(int64_t parameter_number) const;

  // Returns the vector of fused parameters inside this fusion instruction.
  //
  // Precondition: opcode() == NloOpcode::kFusion
  const std::vector<NloInstruction*>& fused_parameters() const;

  // Returns true if this instruction is a fusion instruction that generates
  // multiple outputs.
  const bool IsMultiOutputFusion() const {
    return (opcode() == NloOpcode::kFusion &&
            (fused_expression_root()->opcode() == NloOpcode::kTuple ||
             (fused_expression_root()->opcode() == NloOpcode::kFusion &&
              (fused_expression_root()->fused_expression_root()->opcode() == NloOpcode::kTuple ||
               (fused_expression_root()->fused_expression_root()->opcode() == NloOpcode::kFusion &&
                fused_expression_root()->fused_expression_root()->
                fused_expression_root()->opcode() == NloOpcode::kTuple)))));
  }

  FusionKind fusion_kind() const {
    assert(NloOpcode::kFusion == opcode_);
    return fusion_kind_;
  }
  ...
  void MergeFusionInstruction(NloInstruction* instruction_to_merge);
  ...
  void MergeFusionInstructionIntoMultiOutput(
      NloInstruction* instruction_to_merge);
  ...
  //
  // Precondition: this->opcode() == NloOpcode::kFusion
  NloInstruction* FuseInstruction(NloInstruction* instruction_to_fuse) {
  ...
  //
  // Precondition: this->opcode() == NloOpcode::kFusion
  NloInstruction* FuseInstructionIntoMultiOutput(
      NloInstruction* instruction_to_fuse) {
  ...
  //
  // Precondition: opcode() == NloOpcode::kSlice
  int64_t slice_starts(int64_t dimension) const {
    assert(NloOpcode::kSlice == opcode_);
  ...
  // slice node.
  //
  // Precondition: opcode() == NloOpcode::kDynamicSlice
  int64_t slice_sizes(int64_t dimension) const {
    assert(NloOpcode::kDynamicSlice == opcode_);
  ...
  //
  // Precondition: opcode() == NloOpcode::kReducePrecision
  int32_t exponent_bits() const {
    assert(NloOpcode::kReducePrecision == opcode_);
  ...
  std::unique_ptr<NloInstruction> Clone(const string& suffix = "clone") const;

  // Clones the HLO instruction as above but with new shape and operands.
  std::unique_ptr<NloInstruction> CloneWithNewOperands(
      const NloShape& shape,
      std::vector<NloInstruction*> operands) const;

  // Returns the computations this instruction directly calls (if any).
  const std::vector<NloComputation*>& called_computations() const {
  ...
  void ReplaceCalledComputations(
      std::function<NloComputation*(NloComputation*)> map_function) {
  ...
  bool ImplicitlyBroadcastsOperand(int64_t operand_idx) const;
  ...
  // multiple times.
  std::vector<int64_t> OperandIndices(const NloInstruction* operand) const;
  ...
  // NloOpcodeString plus, for fusion nodes, the fusion kind, separated by a
  // ':'.
  ...
  void SetUniqueId(int id) {
    assert(unique_id_ == -1);  // Should not be assigned already
    assert(id >= 0);
    unique_id_ = id;
  }
  ...
  // be called by NloComputation methods which add/remove instructions to
  // computations.
  void set_parent(NloComputation* computation) { parent_ = computation; }
  const NloComputation* parent() const { return parent_; }
  NloComputation* parent() { return parent_; }

  // Returns the module for this instruction.
  NloModule* GetModule() const;
 ...
 private:
  ...
  //
  // Precondition: this->opcode() == NloOpcode::kFusion
  NloInstruction* FuseInstructionInternal(NloInstruction* instruction_to_fuse,
  ...
  //
  // Precondition: opcode() == NloOpcode::kFusion
  NloInstruction* CloneAndFuseInternal(NloInstruction* instruction_to_fuse,
                                       bool add_output = false);

  // Clones a fusion instruction with a new shape and operands.
  std::unique_ptr<NloInstruction> CloneFusionWithNewOperands(
      const NloShape& shape,
      std::vector<NloInstruction*> operands) const;

  // Returns how this instruction uses elements of its `i`th operand.
  UseKind OperandElementUse(int64_t i) const;

  int unique_id_;  // Unique to this NloInstruction within a NloModule

  // Opcode for this instruction.
  NloOpcode opcode_;

  // Instruction operands.
  std::vector<NloInstruction*> operands_;

  // The set of control predecessors of this instruction.
  std::vector<NloInstruction*> control_predecessors_;
  ...
  std::vector<NloInstruction*> users_;
  std::unordered_set<const NloInstruction*> user_set_;

  // The set of control successors of this instruction.
  std::vector<NloInstruction*> control_successors_;

  // The computation in which this instruction is contained.
  NloComputation* parent_ = nullptr;

  // NloShape of outfeed request.
  NloShape outfeed_shape_;

  // Result shape of this instruction.
  NloShape shape_;

  // Literal, only present for kConstant.
  std::unique_ptr<NloLiteral> literal_;
  ...
  std::vector<NloComputation*> called_computations_;
  ...
  // an operand.
  NloInstruction* trace_instruction_ = nullptr;
...
string ToString(NloInstruction::FusionKind kind);

std::ostream& operator<<(std::ostream& os, NloInstruction::FusionKind kind);

}  // namespace sinian

#endif  // NLO_INTERFACE_NLO_INSTRUCTION_H_
