#external_dir will be absolute_path of current_dir/../external
export TEST_TMPDIR=$(dirname `readlink -f $0`)/../_external
EXTERNAL_DIR=$(dirname `readlink -f $0`)/../_external
echo $EXTERNAL_DIR
#prepare cuda-12.1
if [ ! -d $EXTERNAL_DIR/usr/local/cuda-12.1 ]; then
    rm -rf cuda-12.1*
    wget http://dongjunchen.oss-cn-hangzhou-zmf.aliyuncs.com/cuda12/cuda-12.1_cudnn8.9.6_trt8.6.1.tar.gz -O cuda-12.1.tar.gz
    tar -xzvf cuda-12.1.tar.gz
    cp -r -d cuda-12.1_cudnn8.9.6_trt8.6.1 $EXTERNAL_DIR/usr/local/cuda-12.1
    ln -s $EXTERNAL_DIR/usr/local/cuda-12.1 /usr/local
fi
#python ./configure.py
export TF_CUDA_PATHS=$EXTERNAL_DIR/usr/local/cuda-12.1/
export CUDA_TOOLKIT_PATH=$EXTERNAL_DIR/usr/local/cuda-12.1/
export TF_CUDA_VERSION=12.1
export TF_CUDALIB_VERSION=12
export LD_LIBRARY_PATH=$EXTERNAL_DIR/usr/local/cuda-12.1/lib64:$EXTERNAL_DIR/usr/local/cuda-12.1/lib64/stubs:$EXTERNAL_DIR/usr/local/cuda-12.1/extras/CUPTI/lib64:$EXTERNAL_DIR/usr/local/cuda-12.1/targets/x86_64-linux/lib/:$LD_LIBRARY_PATH


export CUDNN_INSTALL_PATH=$EXTERNAL_DIR/usr/local/cuda-12.1/
export TF_CUDNN_VERSION=8.9.6

export NCCL_INSTALL_PATH=$EXTERNAL_DIR/usr/local/cuda-12.1/
...
if [ -f ".tf_configure.bazelrc.cuda12" ]; then
    cp .tf_configure.bazelrc.cuda12 .tf_configure.bazelrc
    echo 'dongjunchen find tf_configure.bazelrc.cuda12'
else
    python ./configure.py
fi
## now loop through the above array
export TEST_TMPDIR=$(dirname `readlink -f $0`)/../_external/_bazel_$(whoami)/cuda12_cache
for target in "${targets[@]}"
do
    bazel build -c opt --copt -g --copt=-mavx2 --config=cuda12 --copt -mfpmath=both --copt -mfma --copt -msse4.2 --copt -D_GLIBCXX_USE_CXX11_ABI=0 --copt -DGOOGLE_CUDA=1  $target
done

EXTERNAL_DIR="../_external"
EXTERNAL_DIR=`readlink -f $EXTERNAL_DIR`
HEADER_DIR=$EXTERNAL_DIR"/usr/local/include/tf_cuda12"

basename means last dir
CURRENT_DIR=`basename $PWD`
BAZEL_CACHE_DIR=`readlink bazel-$CURRENT_DIR`/../../
BAZEL_EXTERNAL_DIR=$BAZEL_CACHE_DIR"/external/"

mkdir -p $EXTERNAL_DIR/usr/local/lib64/tf_cuda12
for target in "${install_targets[@]}"
do
    IFS='/' read -ra path <<< "$target"
    rm $EXTERNAL_DIR/usr/local/lib64/tf_cuda12/${path[-1]} -f
    cp -f bazel-bin/$target $EXTERNAL_DIR/usr/local/lib64/tf_cuda12/
done
...
cp .tf_configure.bazelrc .tf_configure.bazelrc.cuda12
