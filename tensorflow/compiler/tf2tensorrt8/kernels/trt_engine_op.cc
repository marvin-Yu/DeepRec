...
#include "tensorflow/compiler/tf2tensorrt8/convert/convert_nodes.h"
#include "tensorflow/compiler/tf2tensorrt8/convert/utils.h"
#include "tensorflow/compiler/tf2tensorrt8/utils/trt_allocator.h"
#include "tensorflow/compiler/tf2tensorrt8/utils/trt_engine_utils.h"
#include "tensorflow/compiler/tf2tensorrt8/utils/trt_logger.h"
#include "tensorflow/compiler/tf2tensorrt8/utils/trt_lru_cache.h"
...
class TRTEngineOp : public AsyncOpKernel {
 ...
 private:
  ...
  StatusOr<TrtUniquePtrType<nvinfer1::ICudaEngine>> BuildEngine(
      ...
      TRTEngineCacheResource* cache_resource, int64_t* total_flops);
...
StatusOr<TrtUniquePtrType<nvinfer1::ICudaEngine>> TRTEngineOp::BuildEngine(
  ...
  auto status = convert::ConvertGraphDefToEngine(
      ...
      &cache_resource->profiles_, name(), total_flops);
  if (!status.ok()) {
    {
      static int log_counter = 0;
      if (log_counter < 5) {
...
StatusOr<std::pair<EngineContext*, int>> TRTEngineOp::GetEngine(
  ...
  if (static_engine_) {
    ...
    if (!static_engine) {
      ...
      auto result = BuildEngine(input_concrete_shapes, batch_size,
                                ...
                                /*calibrator=*/nullptr, cache_res, &total_flops);
      if (!result.ok()) {
        return std::pair<EngineContext*, int>(&empty_context, 0);
      }
  ...
  int profile_id = -1; 
  if (!use_implicit_batch_) {
    profile_id = cache_res->profiles_.GetProfileNumber(engine_input_shapes);
  ...
  if (use_implicit_batch_) {
    engine_contexts = cache_res->GetEngineContext(engine_input_shapes); 
  } else {
  ...
  if (engine_contexts == nullptr) {
    ...
    LOG(INFO) << "Building a new TensorRT engine for " << name()
              << " with input shapes: " << DebugString(engine_input_shapes)
    ...
    auto result = BuildEngine(engine_input_shapes, engine_batch_size,
    ...
    VLOG(1) << "Added new engine to cache of " << name()
            << ". Cache size: " << cache.size();
    engine_contexts = cache.at(engine_input_shapes).get();
    // Query which profile of the new engine matches the actual input.
    profile_id = cache_res->profiles_.GetProfileNumber(engine_input_shapes);
    // Get Flops:
    if (flops_ != total_flops) {
