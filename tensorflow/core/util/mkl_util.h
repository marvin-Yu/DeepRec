...
#include "tensorflow/core/util/onednn_threadpool.h"
...
inline dnnl::stream* CreateStream(OneDnnThreadPool* eigen_tp,
                                  const engine& engine) {
...
inline void ExecutePrimitive(const std::vector<primitive>& net,
  ...
  // in oneDNN.
  OneDnnThreadPool eigen_tp;
  if (context != nullptr) {
    Eigen::ThreadPoolInterface* eigen_interface =
        EigenThreadPoolFromTfContext(context);
    eigen_tp = OneDnnThreadPool(eigen_interface, ThreadPoolUseCallerThread());
...
// Set a dummy DNNL shape (called when the output is in TF format)
...
class MklDnnData {
 ...
 public:
  ...
  /// supported by DNNL. E.g., DNNL does not support tensor format for 6
    ...
    if (IsReorderNeeded(op_md)) {
      ...
      if (context != nullptr) {
        Eigen::ThreadPoolInterface* eigen_interface =
            EigenThreadPoolFromTfContext(context);
        eigen_tp =
            OneDnnThreadPool(eigen_interface, ThreadPoolUseCallerThread());
...
inline bool IsPrimitiveRefPath(dnnl::primitive_desc_base& base_desc) {
