...
const TensorSliceSet *TensorSliceReader::GetTensorSliceSet(const string &tensor_name) const {
  mutex_lock l(mu_);
  return gtl::FindPtrOrNull(tensors_, tensor_name);
}

Status TensorSliceReader::GetTensorSlice(
        const string &name,
        ...
        std::unique_ptr<tensorflow::Tensor> *out_tensor) const
{
  ...
  TensorShape shape;
  {
    ...
    type = tss->type();
    auto full_shape = tss->shape();
    auto s = slice.SliceTensorShape(full_shape, &shape);
    if (!s.ok()) {
      return s;
    }
  }
