 ...
 private:
  ...
  void Setup(const MklBatchNormFwdParams& fwdParams) {
    ...
    if (IS_SCALE_AND_SHIFT_FLAG_SET) {
      ...
      context_.scale_shift_mem.reset(new MEMORY_CONSTRUCTOR_USING_MEM_PD(
...
class MklFusedBatchNormBwdPrimitive : public MklPrimitive {
  ...
  void Execute(const T* src_data, const U* mean_data, const U* variance_data,
    ...
    if (IS_SCALE_AND_SHIFT_FLAG_SET) {
      ...
      context_.diff_scale_mem->set_data_handle(
          static_cast<void*>(const_cast<U*>(diff_scale_data)));
      context_.diff_shift_mem->set_data_handle(
          static_cast<void*>(const_cast<U*>(diff_shift_data)));
#endif  // !ENABLE_ONEDNN_V3
    }
 ...
 private:
  ...
  void Setup(const MklBatchNormBwdParams& bwdParams) {
...
#else
    auto scale_shift_desc =
        memory::desc({bwdParams.depth}, MklDnnType<U>(), MEMORY_FORMAT::x);
#endif  // !ENABLE_ONEDNN_V3
    ...
    context_.src_mem.reset(new MEMORY_CONSTRUCTOR(
        context_.bwd_pd.get()->PRIMITIVE_DESC_SRC, cpu_engine_, DummyData));
    context_.diff_dst_mem.reset(
        new MEMORY_CONSTRUCTOR(context_.bwd_pd.get()->PRIMITIVE_DESC_DIFF_DST,
                               cpu_engine_, DummyData));
...
#ifndef ENABLE_ONEDNN_V3
    context_.scale_shift_mem.reset(new MEMORY_CONSTRUCTOR_USING_MD(
        scale_shift_desc, cpu_engine_, DummyData));
    context_.diff_scale_shift_mem.reset(new MEMORY_CONSTRUCTOR_USING_MD(
        diff_scale_shift_desc, cpu_engine_, DummyData));
#else
    context_.scale_mem.reset(
    ...
    context_.diff_src_mem.reset(
        new MEMORY_CONSTRUCTOR(context_.bwd_pd.get()->PRIMITIVE_DESC_DIFF_SRC,
                               cpu_engine_, DummyData));
