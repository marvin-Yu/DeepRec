...
class ParallelIndicatorMatMulOpTest(test.TestCase):
    ...
    def testCPUCorrect(self):
        def Compare(parallel, batch_a, batch_b, m, n, k):
            ...
            ind_in = np.random.randint(0, batch_a, size=batch_b, dtype=np.int64)
            tol = 1e-4
            with self.cached_session(use_gpu=False) as sess:
                ...
                z0 = math_ops.parallel_indicator_mat_mul(x_ph, y_ph, ind_ph, parallel)
                z0_val = sess.run(z0, feed_dict={x_ph:x_in, y_ph: y_in, ind_ph: ind_in})
    ...
    def testCPUCorrect(self):
        def Compare(parallel, batch_a, batch_b, m, n, k, use_tanh):
            ...
            with self.cached_session(use_gpu=False) as sess:
                ...
                y_ph = array_ops.placeholder(y_in.dtype)
                ind_ph = array_ops.placeholder(ind_in.dtype)
                z0 = math_ops.parallel_indicator_batched_small_mat_mul(x_ph, y_ph, ind_ph, parallel, use_tanh=use_tanh)
                z0_val = sess.run(z0, feed_dict={x_ph:x_in, y_ph: y_in, ind_ph: ind_in})
            z1 = self._npIndicatorBatchedSmallMatMul(x_in, y_in, ind_in, use_tanh)
            self.assertAllClose(z0_val, z1, rtol=tol, atol=tol)
